class Song {
  ...
  //CONSTRUCTOR
  Song(PApplet sketch, String n, String aw) {
    this.name = n;
    this.artWork = aw;
    this.clicked = false;
  ...
  void printSongs(PApplet sketch, int x, int y) { //printing the song name
    ...
    float fontSize = 12; //base font size
    int charsRemoved = 0;
    sketch.textSize(fontSize);

    String tempDisplayName = displayName;

    while (sketch.textWidth(tempDisplayName) > 105 && tempDisplayName.length() > 0) {
      charsRemoved++;
      tempDisplayName = displayName.substring(0, displayName.length() - charsRemoved) + ("...");
    }
